...
export async function persistIdeaAttachments(
  ...
  for (const file of files) {
    // Server-side re-check (defense in depth) using the shared validator so
    // size/type rules match the client exactly.
    const invalid = validateUploadFile(file);
    if (invalid) {
      return { ok: false, uploaded, error: `${invalid}:${file.name}` };
    }
